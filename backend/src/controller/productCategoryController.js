import { ProoductCategory } from "../model/index.js";
...
const index = async (req, res) => {
  try {
    ...
    const { rows: productCategories, count } =
      await ProoductCategory.findAndCountAll({
...
const create = async (req, res) => {
  try {
    const { name } = req.body;
    const existingProductCategory = await ProoductCategory.findOne({
      where: { [Op.iLike]: name },
    });
    ...
    const productCategory = await ProoductCategory.create({ name });
    return res.status(200).json({
      meta: {
        code: 200,
...
const show = async (req, res) => {
  try {
    const { id } = req.params;
    const productCategory = await ProoductCategory.findByPk(id);
    if (!productCategory) {
